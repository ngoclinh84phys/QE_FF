...
SUBROUTINE c_bands_nscf( )
  ...
  USE mp,                   ONLY : mp_sum
  USE check_stop,           ONLY : check_stop_now
  USE kc_mod,               ONLY : ln_kc_proj, ln_kc_diag, use_wannier_orbs
  !
  IMPLICIT NONE
  !
  ...
  k_loop: DO ik = ik_+1, nks
     ...
     IF (ln_kc_diag) THEN
        ...
        IF (use_wannier_orbs) THEN
           !
           CALL read_orbs_spin ( nbnd, npwx, evc, current_spin, '.wan.occ.emp' )
           !
        ENDIF
        !
     ENDIF
     ...
     IF (ln_kc_proj) THEN
        ...
        IF (lsda) THEN
           !
           call build_nksic_projection ( nbnd, ngk(ik), current_spin, evc, .True. )
           !
        ELSE
           !
           IF (ik == 1) THEN
           ...
           ELSE
              !
              call build_nksic_projection ( nbnd, ngk(ik), current_spin, evc, .False. )
              !
           ENDIF 
           !
        ENDIF
