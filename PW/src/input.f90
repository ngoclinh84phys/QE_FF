...
SUBROUTINE iosys()
  ...
  USE kc_mod,                ONLY : ln_kc_diag, ln_kc_proj                  ! NLN
  USE kc_mod,                ONLY : use_wannier_orbs_  => use_wannier_orbs  ! NLN
  !USE kc_mod,                ONLY : localized_orb_file_=> localized_orb_file! NLN
  USE input_parameters,      ONLY : use_wannier_orbs                        ! NLN
  !USE input_parameters,      ONLY : localized_orb_file                      ! NLN
  ! 
  IMPLICIT NONE
  !
  INTERFACE  
...
!!!!  
  ...
  CASE( 'bands' )
     ...
     nstep  = 1
     !
  CASE( 'kc_correction' ) ! NLN
     !
     ln_kc_proj = .false. ! NLN
     ln_kc_diag = .true.  ! NLN
     lforce = .false.     ! NLN 
     nstep  = 1           ! NLN
     use_wannier_orbs_   = use_wannier_orbs   ! NLN
     !localized_orb_file_ = localized_orb_file ! NLN
     !
  CASE( 'relax' )
     !
     lscf   = .true.
