...
!
        ...
        DATA calculation_allowed / 'scf', 'nscf', 'kc_correction', 'relax', 'md', 'cp', &
...
!
        ...
        LOGICAL  :: finite_field_pert = .false.
          ! if true, the code perform an finite field  pertubation calculation  ! NLN

        LOGICAL  :: finite_field_bse = .false.
          ! if true, the code perform an finite field bse pertubation calculation  ! NLN
 
        LOGICAL  :: finite_field_kc = .false.
          ! if true, the code perform an finite field kc pertubation calculation  ! NLN
 
        LOGICAL  :: use_wannier_orbs = .false.
          ! if true, the finite field pertubing potential will be computed ontop wannier orbs ! NLN
          ! if false, the finite field pertubing potential will be computed ontop KS orbs ! NLN
          !
        CHARACTER(len=256) :: localized_orbs_file
          ! the file name stores the localized orbitals on which koopmans correction is computed 
          !
        INTEGER  :: orb_index_i = 1
          ! index orbital_v for building the finite field pertubing potential  ! NLN

        INTEGER  :: orb_index_j = 1
          ! index orbital_v' for building the finite field pertubing potential ! NLN
          !
        REAL(DP) :: overlap_thr = 0
          ! the thredhold value to define overlap between orbitals  
        ...
        NAMELIST / system / ibrav, celldm, a, b, c, cosab, cosac, cosbc, nat, &
             ...
             finite_field_pert, finite_field_bse, finite_field_kc,            &     ! NLN
             use_wannier_orbs, orb_index_i, orb_index_j, overlap_thr                ! NLN
