...
PROGRAM pw2wannier90
  ...
  CHARACTER(len=256) :: extension_ln
  INTEGER :: nbnd_computed_occ, nbnd_computed_emp
  ...
  NAMELIST / inputpp / outdir, prefix, spin_component, wan_mode, &
   ...
   ! end change Lopez, Thonhauser, Souza
       ...
       extension_ln,& ! change Nguyen LN
       nbnd_computed_occ,& ! change Nguyen LN
       nbnd_computed_emp ! change Nguyen LN
  !
  ...
  CALL mp_bcast(extension_ln,ionode_id, world_comm)
  CALL mp_bcast(nbnd_computed_occ,ionode_id, world_comm)
  CALL mp_bcast(nbnd_computed_emp,ionode_id, world_comm)
  !
...
!     seedname='wannier'
  ...
  IF (wan_mode.eq.'wannier2odd') THEN
     ...
     !call read_nnkp
     call openfil_pp
     if (spin_component == 'none') ispinw = 1
     call wan2odd( seedname, ispinw, iknum, ikstart, extension_ln)
     call stop_pp
     !
  ENDIF
  !
  IF (wan_mode.eq.'wannier2odd_com') THEN
     !
     call openfil_pp
     if (spin_component == 'none') ispinw = 1
     call wan2odd_combine( nbnd_computed_occ, nbnd_computed_emp, ispinw, extension_ln)
     call stop_pp
     !
  ENDIF
  !
