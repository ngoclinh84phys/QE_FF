...
SUBROUTINE end_finite_field_method ()
  ...
  IF (l_ff_bse) THEN
     ...
     ! 
     fname = "E"//TRIM(ADJUSTL(my_labeli))//"_"//TRIM(ADJUSTL(my_labelj))//".dat"
     !
...
write(stdout,*) 'ij', nbnd_all, nbnd_val_i, nbnd_val_j
  ...
  IF (use_wannier_orbs) THEN
     ...
     IF (nspin == 1) THEN
        !
        CALL read_orbs_spin ( nbnd, npw, evc_init(:,1:nbnd), 1, '.occ' )
        !
     ENDIF
     !
     IF (nspin == 2) THEN
        !
        CALL read_orbs_spin ( nbnd, npw, evc_init(:,1:nbnd), 1, '.occ' )
        CALL read_orbs_spin ( nbnd, npw, evc_init(:,nbnd+1:nbnd_all), 2, '.occ' )
        !
     ENDIF
     !
  ELSE
...
SUBROUTINE orbital_density_kc (fixed_band_i, fixed_band_j, drhog, drhor )
  ...
  IF (use_wannier_orbs) THEN
     ...
     IF (nspin == 1) THEN
        !
        CALL read_orbs_spin ( nbnd_all, npw, evc_init(:,1:nbnd_all), 1, '.occ.emp' )
        !
     ENDIF
     !
     IF (nspin == 2) THEN
        ...
        CALL read_orbs_spin ( nbnd_all2, npw, evc_init(:,1:nbnd_all2), 1, '.occ.emp' )
        CALL read_orbs_spin ( nbnd_all2, npw, evc_init(:,nbnd_all2+1:nbnd_all), 2, '.occ.emp' )
        !
     ENDIF
     !
  ELSE
     ...
     IF (nspin == 1) THEN
        !
        CALL read_orbs_spin ( nbnd_all, npw, evc_init(:,1:nbnd_all), 1, '.ks.occ.emp' )
        !
     ENDIF
     !
     IF (nspin == 2) THEN
        !
        nbnd_all2 =  nbnd_all/2
        CALL read_orbs_spin ( nbnd_all2, npw, evc_init(:,1:nbnd_all2), 1, '.ks.occ.emp' )
        CALL read_orbs_spin ( nbnd_all2, npw, evc_init(:,nbnd_all2+1:nbnd_all), 2, 'ks.occ.emp' )
        !
     ENDIF
     !
  ENDIF 
