...
!
!----------------------------------------------------------------------------
SUBROUTINE run_pwscf_ff ( exit_status ) 
  !----------------------------------------------------------------------------
  !
  ...
  USE finite_field_mod, ONLY : finite_field_pert ! NLN
  !
  ...
  main_loop: DO idone = 1, nstep
     ...
     !
     IF (finite_field_pert) THEN  !NLN
        !
        CALL initialize_finite_field_method(l_do_ff) ! NLN
        !   
        IF (.NOT.l_do_ff) THEN ! NLN 
           !
           stop
           !  
        ENDIF ! NLN 
     ...
     END IF
     !
     IF (finite_field_pert) CALL end_finite_field_method() ! NLN
     !
