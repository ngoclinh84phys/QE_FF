...
! Marco Govoni
! Linh adapts it to BSE i/o 
!
!-----------------------------------------------------------------------
MODULE  io_bsepot_xml
  !----------------------------------------------------------------------------
  !
  USE iotk_module
  USE kinds,       ONLY : DP
  ...
  USE gvect,       ONLY : ngm, ngmx, ig_l2g
  !
  IMPLICIT NONE
  !
  CONTAINS
    ...
    ! ******************************************
    ! WRITE IN G SPACE 
    !       dvrho is passed distributed in G space
    ...
    SUBROUTINE pdep_merge_and_write_G(fname,pdepg)
      ...
      COMPLEX(DP), INTENT(IN)  :: pdepg(ngmx)
      !
      ! Scratch
      !
      ...
      !
      ngm_g = MAXVAL(ig_l2g(1:ngm))
      CALL mp_max(ngm_g, intra_bgrp_comm)
      !
      ! Resume all components 
      !
      ALLOCATE( tmp_vec(ngm_g) )
      tmp_vec=0._DP
      !
      CALL mergewf( pdepg(:), tmp_vec, ngm, ig_l2g(1:ngm), me_bgrp, nproc_bgrp, root_bgrp, intra_bgrp_comm) 
      !
      ...
      IF(me_bgrp==root_bgrp) THEN 
         ...
         CALL iotk_write_dat ( iun, "ndim" , ngm_g )
         CALL iotk_write_dat ( iun, "pdep" , tmp_vec(1:ngm_g) )
    ...
    SUBROUTINE pdep_read_G_and_distribute(fname,pdepg)
      ...
      !    
      CHARACTER(*), INTENT(IN) :: fname
      COMPLEX(DP), INTENT(OUT) :: pdepg(ngmx)
      !
      ! Scratch
      !
      COMPLEX(DP),ALLOCATABLE :: tmp_vec(:)
      ...
      IF(my_pool_id==0.AND.my_bgrp_id==0) THEN
         ...
         IF(me_bgrp==root_bgrp) THEN 
            ...
            CALL iotk_scan_dat( iun, "pdep" , tmp_vec(1:ngm_g) )
            CALL iotk_scan_end( iun, 'PDEP_GSPACE' )
            CALL iotk_close_read( iun )
            !
         END IF
         !
         CALL splitwf( pdepg, tmp_vec, ngm, ig_l2g(1:ngm), me_bgrp, nproc_bgrp, root_bgrp, intra_bgrp_comm) 
         !
      ENDIF
      !
      DEALLOCATE( tmp_vec )
